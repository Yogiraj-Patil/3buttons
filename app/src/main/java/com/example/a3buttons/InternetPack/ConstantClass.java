package com.example.a3buttons.InternetPack;

public class ConstantClass {
    public static String PREF = "POLICYAPP";
    //private static String BASE_URL = "https://yogirajpatil.gq/policy_app/";
    private static String BASE_URL = "http://192.168.0.200/policy/";
    public static String Login = ""+BASE_URL+"login.php?";
    public static String Insert_record = ""+BASE_URL+"insert_policy.php/?";
    public static String Search_record = ""+BASE_URL+"search_policy.php/?";
}
